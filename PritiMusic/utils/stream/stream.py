...
async def stream(
    ...
    if streamtype == "playlist":
        ...
        for search in result:
            ...
            if await is_active_chat(chat_id):
                ...
                run = await app.send_photo(
                    ...
                    reply_markup=InlineKeyboardMarkup(button), has_spoiler=True
                )
        ...
        if count == 0: return
            ...
            return await app.send_photo(original_chat_id, photo=carbon, caption=_["play_21"].format(position, link), reply_markup=close_markup(_), has_spoiler=True)
        ...
        if await is_active_chat(chat_id):
            ...
            run = await app.send_photo(original_chat_id, photo=img, caption=_["stream_1"].format(f"https://t.me/{app.username}?start=info_{vidid}", title[:23], duration_min, user_name), reply_markup=InlineKeyboardMarkup(stream_markup(_, chat_id)), has_spoiler=True)
            db[chat_id][0]["mystic"] = run
            db[chat_id][0]["markup"] = "stream"
    ...
    # ... (Other streamtypes: soundcloud, telegram, index follow similar logic)
